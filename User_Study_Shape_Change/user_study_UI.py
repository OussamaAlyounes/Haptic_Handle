...
class User_Study:
    def __init__(self):
        ...
        self.label_command = tk.Label(self.my_window, textvariable=self.label_command_text, font=font)
        self.label_starting_point = tk.Label(self.frame_opening, text="Starting point:", font=font)
    ...
    def terminate_error(self):
        ...
        messagebox.showwarning("OOPS", message="You are either overly sensetive or abnormally unsensitive!\n"+"The experiment was terinated.")
        self.my_window.destroy()

    def terminate(self):
        messagebox.showinfo("Congrats!", message="Thank you for completing the experiment successfully")
        # Save the data in a file
        np.savez(file = f"{self.name}_{self.option_starting_position_selected.get()}.npz", time_start = self.time_start, time_end = datetime.datetime.now(), 
    ...
    def window_open(self):
        ...
        self.label_command.place(x=self.window_width//2-80, y=self.window_height//2-50)
        self.entry_name.place(x=self.window_width//2, y=self.window_height//2-50, height= 30)

        self.label_starting_point.place(x=self.window_width//2-80, y=self.window_height//2)
        self.option_starting_position.place(x=self.window_width//2+40, y=self.window_height//2-5)

        self.button_save.place(x=(self.window_width-self.button_save.winfo_reqwidth())//2, y=(self.window_height-self.button_save.winfo_reqheight())//2+100)
    ...
    def action_save(self):
        # save data
        if self.option_starting_position_selected.get() == 'min':
            self.direction = 1
    ...
    def action_start(self):
        ...
        self.label_command_text.set("Put your hand OFF the device and press Test ('t') to test the noise cancelation.")
        ...
        # time.sleep(2)
        # self.label_command_text.set("wrong.")
    ...
    def test_motors(self):
        ...
        self.button_test_noise.place(x=(self.window_width-self.button_test_noise.winfo_reqwidth())//2, y=(self.window_height-self.button_test_noise.winfo_reqheight())//2)
        self.label_command_text.set("Keep adjusting the volume manually and then press Test ('t') again.\n"
                                    +"When you stop hearing anything, press Continue ('c').") # Adjusted to be label (DONE!)
        self.label_command.place(x=(self.window_width-self.label_command.winfo_reqwidth())//2, y=(self.window_height-self.label_command.winfo_reqheight())//2-50)
    ...
    def action_continue(self, from_testing = True):
        ...
        self.label_command_text.set("Please open your hand and then press Go ('g').")
        self.label_command.place(x=(self.window_width-self.label_command.winfo_reqwidth())//2, y=(self.window_height-self.label_command.winfo_reqheight())//2-50)
    ...
    def action_trial(self):
        if self.repeated_go == 0: # move motor to baseline and ask to close the hand
            ...
            # threading.Thread(target=self.send_motor, args=(self.pose_motor_base,)).start()
            # self.my_window.after(1000, lambda: self.send_motor(self.pose_motor_base))
            # print("I am here")
            # time.sleep(2)
            self.label_command_text.set("Please hold the device and then press Go ('g').")
            self.label_command.place(x=(self.window_width-self.label_command.winfo_reqwidth())//2, y=(self.window_height-self.label_command.winfo_reqheight())//2-50)
            self.button_go.place(x=(self.window_width-self.button_go.winfo_reqwidth())//2, y=(self.window_height-self.button_go.winfo_reqheight())//2)

        elif self.repeated_go == 1: # ask to open the hand
            self.label_command_text.set("Remember how it feels... open your hand and then press Go ('g').")
            self.label_command.place(x=(self.window_width-self.label_command.winfo_reqwidth())//2, y=(self.window_height-self.label_command.winfo_reqheight())//2-50)

        else: # move to the next frame (responding)
            # hide the old frame (commanding) without destroying it
            ...
            self.label_command_text.set("Close your hand again to feel the new position.\n"+"Was the position different from the previous one?")
            self.label_command.place(x=(self.window_width-self.label_command.winfo_reqwidth())//2, y=(self.window_height-self.label_command.winfo_reqheight())//2-50)
