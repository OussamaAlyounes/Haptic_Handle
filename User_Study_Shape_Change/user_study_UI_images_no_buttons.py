...
class User_Study:
    def __init__(self):
        ...
        self.label_sex = tk.Label(self.frame_opening, text= "Sex", font=font)
        self.label_dominant_hand = tk.Label(self.frame_opening, text= "Dominant Hand", font = font)
        self.label_hand_length = tk.Label(self.frame_opening, text= "Hand Length", font=font)
        self.label_finger_length = tk.Label(self.frame_opening, text= "Finger Length", font=font)

        # texts (instead of labels)
        self.text_command = tk.Text(self.my_window, height=10, width=70, bd = 0, font=font) # bd =0 to remove the boarder
        self.text_command.config(background=self.my_window.cget("bg")) # to make it the same background of the window

        # entries
        self.entry_name = tk.Entry(self.frame_opening, width= 20, font=font-10)
        self.entry_hand_length = tk.Entry(self.frame_opening, width= 5, font=font-10)
        self.entry_finger_length = tk.Entry(self.frame_opening, width= 5, font=font-10)
        ...
        self.option_sex.config(font=font-5)
        ...
        self.option_dominant_hand.config(font=font-5)

        # buttons and bindings
        self.button_save = tk.Button(self.frame_opening, text= "Save", command=self.action_save, font=font, bg="skyblue")
        self.button_start = tk.Button(self.frame_opening, text= "Start", command=self.action_start, font = font, bg="green")
    ...
    def terminate(self, Fail = False):
        ...
        np.savez(file = f"{self.directory}\{self.name}_{self.option_experiment}.npz", time_start = self.time_start, time_end = datetime.datetime.now(), 
    ...
    def action_start(self):
        ...
        self.change_text(["OPEN your hand the device and press Test 't' "], [" to test the noise cancelation."], [self.im_keyboard_t])

    def test_motors(self):
        # time.sleep(2)
        self.send_motor(self.pose_motor_max/2) # 5 mm
        time.sleep(1)
        self.send_motor(self.pose_motor_base) # 0 mm, minimum displacement or completely closed
        time.sleep(1)
